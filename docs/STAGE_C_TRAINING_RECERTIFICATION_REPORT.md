# Final update 2026-09-15 — narrowed C02 training recertification PASS

**Final CPU: PASS. Final CUDA: PASS under the seeded FP32 contract. Overall: PASS in the explicitly accepted narrowed C02 fresh/full-checkpoint training scope.**

This conclusion combines existing successful replay/restore/BEST evidence with the newly completed finite probes. It does not relabel earlier failed attempts. No positive training, replay, BEST reconstruction or checkpoint restore was repeated in this attempt.

**Weights-only: NOT RECERTIFIED AFTER C02 / DEFERRED.** Fresh weights-only export/load is explicitly outside this accepted closure scope. No registry semantics were changed and no weights-only PASS is claimed. This is not PASS against the original unamended DoD.

Archived harness path: the historical `ml/tests/stage_c_recertification/supplement.py` is now preserved byte-for-byte at [supplement_failed.py.txt](../experiments/stage_c_training_recertification/supplement_preparation/supplement_failed.py.txt), SHA256 `e6f17b192e7934aa4929fcd28d17407ab5bfc8dc6beb0aa987b689c43a26655a`. Historical commands and result paths below retain their original spelling; this is no longer an active test entrypoint. The passing harness does not import it.

## New attempt and exact root cause

Run **20260915T153405Z-finite**, revision nmd-c02-finite-containment-2. A single new authorization, six separate sequential workers: one diagnostic, then five finite probes. All exited0; parent exit0. No retry inside this authorization.

The only accepted containment event is the exact blocked **os.mkdir** for already-existing `/home/radian/NeuroMetabolic Dashboard/ml/models/baseline_v1_stage_a`, initiated by train_tft_population_v2 module import, before finite injection. Python pathlib.mkdir handles the PermissionError internally because exist_ok=true and the directory exists. It does not escape the immediate library call or become the worker's final uncaught exception. The audit hook prevents the attempted filesystem operation; original inventory and known-directory identity (device/inode/mode/mtime/ctime) remain unchanged.

Minimal test-only correction removed the site-packages-frame requirement, replacing it with the exact path, event/operation, existing-directory, import-frame, pre-injection and handled-exception checks. No generic acceptance of caught filesystem violations. Each worker recorded exactly this one containment event at seq1. Phase A reproduced it with no injection/forward/backward/optimizer; any different or unexplained event would fail.

All six workers report containment_event_seen=true, containment_uncaught_rejection=false, containment_was_first_uncaught_rejection_after_injection=false. Ordered audit events retain monotonic nanosecond timestamps, paths, stacks and propagation. These distinctions replace the old sticky-flag interpretation. [acceptance_summary.json](../experiments/stage_c_training_recertification/20260915T153405Z-finite/acceptance_summary.json) summarizes them without discarding full event logs.

## Newly executed finite gates

| Worker | Result | Production refusal / diagnostic | Seconds |
|---|---|---|---:|
| diagnostic | PASS | no injection; known event explained | 7.223 |
| CPU_N_target | PASS | Nonfinite state: model input | 9.325 |
| CPU_N_gradient | PASS | Nonfinite state: gradient | 9.726 |
| CUDA_N_input | PASS | Nonfinite state: model input | 10.279 |
| CUDA_N_target | PASS | Nonfinite state: target | 10.279 |
| CUDA_N_gradient | PASS | Nonfinite state: gradient | 10.477 |

All five injections were proven. The first uncaught rejection of each injected state originated in production finite_training.py/require_finite, with the exact accepted FloatingPointError message. No containment rejection preceded it after injection. All five fresh negative runs have no valid BEST/LAST. Base AdamW entry/completion counts remain zero.

CPU target Inf was visible through the input decoder-target alias and safely rejected as model input. CUDA target Inf reached the later target-specific gate. Both satisfy the reviewed causal acceptance; no requirement to force one later message. Gradient injection occurs in a backward hook; production gradient validation rejects it before optimizer delegation on CPU and CUDA. Input/target probes require no backward.

Representative event ordering: CPU target injection28 → production finite exception38/39 → outer production refusal40; CUDA target injection28 → finite exception44 → refusal45; both gradient probes backward54 → gradient injection55 → finite exceptions58/59 → refusal60. Multiple finite exception entries reflect propagation through recursive require_finite calls, not separate retries. The known caught import event is seq1 in every worker. No base AdamW delegation event exists.

## Inherited evidence, not rerun

| Requirement | Evidence source | Closure status |
|---|---|---|
| R0 C02 tensors/oracle, masks/windows, compatibility |20260915T143739Z R0|PASS, inherited|
| CPU strict U8 vs P4+R4 |20260915T143739Z|PASS, inherited|
| CUDA C1/C2/C3 and P4+R4, all six comparisons |20260915T143739Z|PASS, inherited; recorded discrepancies0|
| Historical step4 LAST and full native restore |original CPU_R/CUDA_R|PASS, inherited|
| Current BEST ownership/compatibility and independent selection |20260915T152140Z-closure serialization|PASS, inherited|
| Trained BEST tensors/Q/features/encoder map/GroupNormalizer/dataset/encoder_cont/target_scale/window keys and log-stat oracle |same read-only serialization evidence|PASS, inherited|
| Completed-run LAST refusal |same read-only evidence|PASS, inherited|
| CPU input NaN |original campaign|PASS, inherited|
| Remaining five finite probes |20260915T153405Z-finite|PASS, new|
| Original artifact integrity |inventory after every phase and exit|PASS|
| Weights-only export/load |explicit scope amendment|NOT RECERTIFIED AFTER C02 / DEFERRED|

Previous BEST assertion evidence is not rerun; its earlier containment qualification is explained by the now-reproduced import-time event. The current result makes no new claim about mid-epoch resume, bitwise CUDA continuation, cross-device/cross-stack, AMP, DDP, full model capacity, clinical quality or calibration. SWA OFF; clinical loss unchanged. NO PERFORMANCE EVALUATION.

## Preregistration, budget and immutable history

[preregistration.json](../experiments/stage_c_training_recertification/20260915T153405Z-finite/preregistration.json) SHA256 **aa7c9a54e24e4d6bcf4140b387c94294a4e0ee13236038e40729aa5fd40b2875** records exact source hashes, original inventory, directory identity, the two phases and fixed acceptance. Raw logs/events remain under ignored `ml/models/stage_c_training_recertification/20260915T153405Z-finite/`; lightweight evidence under the matching experiments directory. Existing synthetic R0 fixture was reused; no new candidate windows or real data were generated/read.

| Counter | Actual | Limit |
|---|---:|---:|
| Base AdamW attempts/completions |0/0|0/0|
| Model forwards |10|32|
| Backwards |2|2|
| Candidate windows generated |0|0|
| Workers |6|6|
| Total wall seconds |57.392|300|
| Longest worker seconds |10.477|90|

Phase A:0 forward/backward/optimizer. Only CPU/CUDA gradient probes used backward. Aggregate optimizer consumption across all historical attempts stays **48 attempts/48 completions**, entirely from the original positive campaign. No positive replay was rerun. No numeric tolerances or production flags were changed to obtain PASS.

Host CUDA access was checked in each worker: available, one expected RTX3070. Stack matched Python3.14.7, PyTorch2.11.0+cu130, Lightning2.6.1, PF1.7.0, NumPy2.4.4, pandas2.3.3, CUDA13.0/cuDNN91900. CPU strict and CUDA seeded FP32/IEEE profiles retained. No install/update/reboot.

Original source campaign inventory is unchanged after each worker and final exit; known-directory identity unchanged. Previous report preserved byte-for-byte at report_before.md and below. Prior finite harness source was archived before the minimal correction; all earlier FAIL/INCOMPLETE attempts remain historical, not overwritten.

## Changes and STOP

Only test/harness predicate and diagnostic integrity metadata changed in `ml/tests/stage_c_recertification/finite_closure.py`; new attempt evidence and this report update. No production/config/registry changes, relocation, symlinks, ownership rewrite, exports, real evaluation, Optuna or full Baseline training. Static syntax, source hash and git diff --check verification recorded in final_verification.json.

Branch research/baseline-audit; HEAD2fed05028aaa303ab06362cee96d02c3221fece8; dirty/untracked test/report/evidence, nothing staged, no commit/push. **STOP after report. PASS does not authorize another stage or full training.**

---

Earlier report text preserved unchanged as historical evidence:

# Update 2026-09-15 — containment diagnostic root cause resolved

**Final overall verdict: FAIL / INCOMPLETE.** Attempt `20260915T152904Z-finite` stopped at Phase A acceptance; Phase B was not launched. All counters are zero. No automatic retry or repair followed. The containment ambiguity is now explained, but remaining finite probes are not certified.

**Weights-only: NOT RECERTIFIED AFTER C02 / DEFERRED.** This exclusion remains explicitly accepted; no weights-only PASS or export is claimed.

## Exact containment root cause

One event was recorded: seq1, monotonic_ns5642880746778, audit event **os.mkdir**, resource `/home/radian/NeuroMetabolic Dashboard/ml/models/baseline_v1_stage_a`, operation os.mkdir. The directory already existed.

Call chain: import train_tft_population_v2 → module line104 `MODEL_DIR.mkdir(parents=True, exist_ok=True)` → Python3.14 pathlib.mkdir line1011 → audit hook → PermissionError. Pathlib catches OSError (including PermissionError) and suppresses it when exist_ok=true and self.is_dir() is true (lines1017–1022). The attempted mkdir is blocked before filesystem mutation. This was an import-time attempt, unrelated to injected finite state.

Recorded propagation: exception reached pathlib.mkdir, **did not escape that immediate library call**, and **did not become the worker's final uncaught exception**. No injection, production finite rejection, model forward, backward or base AdamW delegation occurred. Therefore:

- containment_event_seen=true;
- containment_uncaught_rejection=false;
- containment_was_first_uncaught_rejection_after_injection=false (no injection occurred).

The former sticky flag incorrectly treated the existence of this caught diagnostic event as evidence that containment rejected the finite probe. The new event record distinguishes those facts. Full stack and propagation are in [diagnostic_events.json](../experiments/stage_c_training_recertification/20260915T152904Z-finite/diagnostic_events.json).

## Why this attempt still stopped

Phase A required a caught mkdir for an already existing nonsource directory **and a /site-packages/ frame in its stack**. The actual operation originated in the repository module and stdlib pathlib, with no site-packages frame. The test therefore raised **AssertionError: Phase A containment needs review** at finite_closure.py:115.

That additional site-packages condition was an overly narrow harness assumption. The generic event classification UNEXPLAINED_OR_BOUNDARY_VIOLATION in the raw record is its preregistered branch label, not evidence that a file changed. Read-only source review after termination explains the benign existing-directory behavior above. Do not relabel the executed FAIL as PASS; the acceptance criterion was not adjusted after results. The attempt does not demonstrate a production finite-safety defect and does not justify a production change. No Phase B rerun was made.

## Inherited versus new evidence

| Scope | Status | Evidence |
|---|---|---|
| CPU strict replay | PASS, inherited |48-update campaign; not repeated|
| CUDA C1/C2/C3 + resume | PASS, inherited |recorded differences0; no bitwise guarantee|
| Historical LAST boundary restore | PASS, inherited |successful original P→R|
| Current BEST verification | PASS, inherited |read-only closure assertions; not rerun|
| Trained BEST C02 normalizer/encoder reconstruction | PASS, inherited |prior closure tensors/oracle evidence; not rerun|
| Completed LAST refusal | PASS, inherited |prior read-only evidence|
| CPU input NaN | PASS, inherited |original campaign; not repeated|
| Exact containment event capture | Captured/explained |new Phase A event and stdlib/source review|
| Phase A acceptance | FAIL |overly narrow site-packages predicate|
| CPU target Inf / gradient NaN | NOT RUN |Phase B not entered|
| CUDA input / target / gradient | NOT RUN |Phase B not entered|
| Final CPU certification | INCOMPLETE |finite gates pending|
| Final CUDA certification | INCOMPLETE |finite gates pending|
| Overall narrowed C02 recertification | FAIL / INCOMPLETE |no newly closed finite gate|

Historical FAIL/INCOMPLETE records remain unchanged. The current root-cause explanation does not retroactively execute missing probes.

## Instrumentation and preregistration

New test-only `ml/tests/stage_c_recertification/finite_closure.py` records monotonic sequence/timestamps, audit event/resource/operation, stack, exception propagation through the immediate library frame, final exception classification, injection and require_finite/backward/base optimizer ordering. Trace/profile observation does not replace production methods. Expected finite refusals remain exact-type/message/production-origin checks with proven injection, zero base optimizer calls and no valid BEST/LAST. A caught filesystem event alone does not set the uncaught-rejection flags.

[preregistration.json](../experiments/stage_c_training_recertification/20260915T152904Z-finite/preregistration.json), SHA256 `a0c1564589156955a93608f5ad42d0952607e0ce275cfcdd972e5c959c8ebd3e`, includes full original inventory/source hashes, one Phase A plus five conditional Phase B workers, and the frozen acceptance predicate. New unique raw directory `ml/models/stage_c_training_recertification/20260915T152904Z-finite/`.

Executed once on host with GPU access:

```bash
python3 ml/tests/stage_c_recertification/finite_closure.py
```

Phase A matched imports, numerical settings, saved synthetic fixture load, model construction and empty probe-registry initialization. No Trainer.fit or model forward. CUDA availability/count/name check passed for one NVIDIA GeForce RTX3070. Stack matched Python3.14.7/PyTorch2.11.0+cu130/Lightning2.6.1/PF1.7.0/NumPy2.4.4/pandas2.3.3, CUDA13.0/cuDNN91900. No package/driver change.

## Budgets, preservation and STOP
...
| Model forwards |0|32|
| Backwards |0|2|
| Candidates generated |0|0 (fixture reused)|
| Workers |1|6|
| Worker seconds |7.127|90|
| Total seconds |7.187|300|

Worker and parent exit1. Five Phase B workers NOT RUN. Original campaign complete inventory matched after diagnostic and at final exit: byte-identical, no additions/removals. Previous report preserved at report_before.md and unchanged below. No original registry or manifest writes, no relocation/symlinks, no positive training/replay, no BEST rerun, no real-data evaluation, no Optuna. Aggregate historical optimizer budget remains48/48.

Only new test harness, new metadata/raw diagnostic output and this report update. Source/config hashes unchanged. Syntax/diff checks in final_verification.json. Branch research/baseline-audit, HEAD2fed05028aaa303ab06362cee96d02c3221fece8; no staging/commit/push.

**STOP after report.** No post-result acceptance change, repair or retry.

---

Earlier report text preserved unchanged:
